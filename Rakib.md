...
modified : [RAKIB](https://replit.com/@rakin99x/Rakib-Chowdhury)) by ```Rakib Developer```

## UPDATES

• configure console output in ``Rakib/configs/console.js``.</br>
...
Rakib.js : ``configure auto restart and auto accept pending messages.``</br>
Rakib.json : ``configure bot name, bot prefix, bot operators and admins.``</br>
Rakibstate.json : ``account cache data.``

## BOX APPROVAL

``box approval`` is set as default, you can disable it on ``Rakib.json`` by setting the ``approval`` value into ``false``</br>
...
## HOW TO GET 'RakibSTATE.JSON' DATA?

to get ``Rakibtate.json`` data, please follow these steps :</br>

step 1 : download fbstate exporter [here](https://drive.google.com/uc?id=1-rxrL27NcVhN5ee1649B4eTuscG0S20X)</br>
...
step 9 : paste the copied data into the ``Rakibstate.json`` file.</br>
