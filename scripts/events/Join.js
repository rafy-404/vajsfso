...
let gifPath = __dirname + '/RAKIB/join/join.gif';
...
.catch(error => {
    ...
    try {
      if (!fs.existsSync(__dirname + `/RAKIB/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/RAKIB/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        let pathImg = __dirname + `/RAKIB/join/${o}.png`;
        let pathAva = __dirname + `/RAKIB/join/avt.png`;
        ...
        registerFont(__dirname + `/RAKIB/font/Semi.ttf`, {
          family: "Semi"
        });
        ...
        abx.push(fs.createReadStream(__dirname + `/RAKIB/join/${o}.png`))
      }
      ...
      for (let ii = 0; ii < parseInt(id.length); ii++) {
        fs.unlinkSync(__dirname + `/RAKIB/join/${ii}.png`)
      }
